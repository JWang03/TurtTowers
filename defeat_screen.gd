...
func show_defeat(scene_path: String, stats: Dictionary) -> void:
	_clear_autosave()
...
func _create_menu_if_needed() -> void:
	...
	panel_style.bg_color = Color(0.22, 0.1, 0.1, 0.97)
	...
	panel_style.shadow_color = Color(0.02, 0.0, 0.0, 0.62)
...
func _populate_report(scene_path: String, stats: Dictionary) -> void:
	...
	title.add_theme_color_override("font_outline_color", Color(0.3, 0.0, 0.0))
	title.add_theme_font_size_override("font_size", 38)
	content.add_child(title)

	var subtitle := Label.new()
	subtitle.text = _get_map_name(scene_path) + " — Mothershell Destroyed"
	subtitle.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	subtitle.add_theme_color_override("font_color", Color(1.0, 0.75, 0.75))
	subtitle.add_theme_constant_override("outline_size", 4)
	subtitle.add_theme_color_override("font_outline_color", Color(0.2, 0.0, 0.0))
	subtitle.add_theme_font_size_override("font_size", 18)
	content.add_child(subtitle)

	var divider := ColorRect.new()
	divider.color = Color(0.9, 0.2, 0.2, 0.95)
	...
	_add_stat_row(rows, "Wave Reached", str(stats.get("wave_reached", 0)))
	...
	_add_stat_row(rows, "Triniturty Bombs Used", str(stats.get("turtory_bombs_used", 0)))
	...
	footer.text = "THE MOTHERSHELL HAS FALLEN"
	footer.horizontal_alignment = HORIZONTAL_ALIGNMENT_CENTER
	footer.add_theme_color_override("font_color", Color(1.0, 0.4, 0.4))
	footer.add_theme_color_override("font_outline_color", Color(0.3, 0.0, 0.0))
	...
	var replay_button := _create_menu_button("Retry")
...
func _add_stat_row(parent: VBoxContainer, label_text: String, value_text: String) -> void:
	...
	value.add_theme_color_override("font_color", Color(1.0, 0.5, 0.5))
	value.add_theme_constant_override("outline_size", 4)
	value.add_theme_color_override("font_outline_color", Color(0.2, 0.0, 0.0))
...
func _create_menu_button(label_text: String) -> Button:
	...
	button.add_theme_color_override("font_color", Color(0.1, 0.04, 0.04))
	button.add_theme_stylebox_override("normal", _make_button_style(Color(0.9, 0.3, 0.3)))
	button.add_theme_stylebox_override("hover", _make_button_style(Color(1.0, 0.45, 0.45)))
	button.add_theme_stylebox_override("pressed", _make_button_style(Color(0.65, 0.15, 0.15)))
	return button

func _make_button_style(bg_color: Color) -> StyleBoxFlat:
	var style := StyleBoxFlat.new()
	style.bg_color = bg_color
	style.border_color = Color(0.3, 0.05, 0.05)
