...
#func create_bolt(start_pos: Vector2, end_pos: Vector2):
	#self.clear_points()
	#self.add_point(to_local(start_pos))
	#self.add_point(to_local(end_pos))
	#
...
#func create_bolt(start_pos: Vector2, end_pos: Vector2):
	...
	#for i in range(1, segments):
		...
		#
		#var offset = Vector2(randf_range(-jiggle_amount, jiggle_amount), randf_range(-jiggle_amount, jiggle_amount))
		#add_point(mid_point + offset)
	#
	#add_point(end_pos)
	#
	#var tween = create_tween()
